...
docker run  -it -p 8080:8080  --mount type=bind,source="$(pwd)",target=/app mfeurer/auto-sklearn /bin/bash -c "cd app/NepalEarthquakes && pip install -e . && cd src/models && pwd && ls -a && python " 
